...
var objectdetect = (function() {
	...
	var detector = (function() {
		 ...
		 */
		detector.prototype.detect = function(imageData, group, stepSize) {
			...
			for (var i = 0; i < this.numScales; ++i) {
				...
				if (scale == 1) {
					...
					this.scaledGray = objectdetect.rescaleImage(this.gray, this.width, this.height, scale, this.scaledGray);
				}
				...
				for (var j = newRects.length - 1; j >= 0; --j) {
					newRects[j][0] *= this.width / scaledWidth;
					newRects[j][1] *= this.height / scaledHeight;
					newRects[j][2] *= this.width / scaledWidth;
					newRects[j][3] *= this.height / scaledHeight;
				}
